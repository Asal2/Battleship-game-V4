//Author: Asal Sharma Dhungana
//Description: The routes module here creates custom routes for each end points and demonstrates the use of routes using the database manager module
...
router.get("/player1/", function (req, res) {
  try {
    ...
    if (gamecoordinates != "undefined") {
      //if it is not undefined after the split
    ...
    if (gamestatus == "reset") {
      ...
      if (player1score == 0 || player1score == 1) {
        //if player1score is more than 1 after game over
        player1score = 0; //reset it to 0
        res.end();
      } else {
        //else insert the score in the database if it is lower
      ...
      //if the game status is a hit and game is not over
...
router.get("/highscores", function (req, res) {
  try {
    ...
    }; //send resposne of records and highscores
