...
function clearMessages() {
  ...
  if (messageDiv !== null) messageDiv.innerHTML = null;
}
//addMessage(msg) – adds a given text (msg) to the message div.
function addMessage(msg) {
  var messageDiv = document.getElementById("messagediv");
  if (messageDiv !== null) messageDiv.innerHTML += msg + "<br>";
...
function setUsername(userName) {
  ...
  let myFetchParams = {
    //fetch parameters sent through the body
    method: "POST",
    mode: "cors",
    headers: {
...
function socketMoves(move) {
  ...
  if (element !== null && element.tagName.toLowerCase() === "td") {
    if (element.innerHTML === "") {
      ...
      if (move === "missed") changeText(element, style); //change text to missed
